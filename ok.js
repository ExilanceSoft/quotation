...
exports.getQuotationById = async (req, res, next) => {
  try {
    ...
    // Validate essential quotation data
    if (!quotation.creator || !quotation.creator._id) {
      return next(new AppError('Creator information missing in quotation', 400));
    }

    if (!quotation.customer || !quotation.customer._id) {
      return next(new AppError('Customer information missing in quotation', 400));
    }

    // Get creator details with proper population
    ...
    const modelIds = quotation.models.map(m => m.model_id).filter(id => id);
    if (modelIds.length === 0) {
      return next(new AppError('No valid models found in quotation', 400));
    }
    ...
    if (models.length !== modelIds.length) {
      logger.warn(`Some models not found for quotation ${quotationId}`);
    }

    const attachments = await getUniqueAttachmentsForModels(models.map(m => m._id));
    ...
    // Get offers with proper filtering
    const allOffers = await Offer.find({
      isActive: true,
      $or: [
        { applyToAllModels: true },
        { applicableModels: { $in: models.map(m => m._id) } }
      ]
    }).populate({
      path: 'applicableModels',
      select: 'model_name',
      match: { _id: { $ne: null } } // Ensure no null models are populated
    });

    // Process unique offers with proper null checks
    const uniqueOffersMap = new Map();
    allOffers.forEach(offer => {
      if (!offer._id) return; // Skip if no _id
      if (!uniqueOffersMap.has(offer._id.toString())) {
        uniqueOffersMap.set(offer._id.toString(), {
          ...offer.toObject(),
          applicableModels: (offer.applicableModels || []).filter(m => m && m._id)
        });
      }
    });
    const uniqueOffers = Array.from(uniqueOffersMap.values());

    // Process models with proper error handling
    const responseModels = await Promise.all(modelsWithBranchPrices.map(async model => {
      const exShowroomPrice = exShowroomHeader
        ? model.prices.find(p => p.header_id?._id.equals(exShowroomHeader._id))?.value
        : null;
    ...
    // Prepare final response with all null checks
    const response = {
      userDetails: {
        _id: creator._id,
        username: creator.username || '',
      ...
      customerDetails: {
        ...
        mobile1: customer.mobile1 || '',
        mobile2: customer.mobile2 || '',
        finance_needed: quotation.finance_needed || false,
        createdAt: customer.createdAt
      },
      expected_delivery_date: quotation.expected_delivery_date
      ...
      allUniqueOffers: uniqueOffers.map(offer => ({
        ...
        url: offer.url || '',
        createdAt: offer.createdAt,
        applyToAllModels: offer.applyToAllModels || false,
        applicableModels: (offer.applicableModels || []).map(model => ({
          _id: model._id,
          model_name: model.model_name || ''
        }))
      }))
    ...
    next(new AppError('Failed to fetch quotation details', 500));
  }
};
